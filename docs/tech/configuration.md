...
```
docker build -f Dockerfile.prod -t lacpass-identity-manager .
```

#### Run docker image (you need to add .env file as param)

```
docker run --rm --env-file=.env.prod -p 3000:3000 --name node-api lacpass-identity-manager
```
